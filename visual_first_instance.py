...
# Optionally dump pallet -> order lists to text files
...
def safe_mean_utilization(pallet_summary):
    """
    Return the mean utilization (%) if 'Utilization (%)' is present in pallet_summary.
...
def main():
    ...
    for orders_file in ORDERS_FILES:
        ...
        for num_pickers in NUM_PICKERS_LIST:
            ...
            res = run_pipeline(
                ...
                debug=False,
                visualize_pallets=True,   # set to False if you want faster execution
                plot_gantt=False,
            )
            ...
            # Dump pallet -> order lists as a .txt file
            if DUMP_PALLET_ORDER_LISTS:
    ...
    out_xlsx = os.path.join(PROJECT_ROOT, "visual.xlsx")
